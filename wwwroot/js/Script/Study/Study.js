    ...
    js.ajax({
        ...
        success: function (result) {
            ...
            table = js("#tblStudy").DataTable({
                ...
                processing: false,
                columns: [
                    { data: 'studyId', targets: 1, searchable: false },
                    { data: 'versionID', targets: 2, searchable: false },
                    { data: 'protocolTitle', targets: 3, searchable: true },
                    { data: 'protocolCode', targets: 4, searchable: false },
                    { data: 'isActive', targets: 5, searchable: false },
                    { data: 'isDeleted', targets: 6, searchable: false },
                    {
                        data: null,
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            if (pay == payMust) {
                $.ajax({
                    type: "POST",
                    url: APILocal + "OrderDetail/SaveDataOrder",
                    data: formData,
                    contentType: false,
                    processData: false,
